...
8. [Псевдоклассы CSS](#8-Псевдоклассы-CSS)
...
| Псевдокласс CSS      | Когда срабатывает                            | XPath                               |
|:---------------------|:---------------------------------------------|:------------------------------------|
| `:hover`             | Курсор мыши наведён на элемент               | -                                   |
| `:focus`             | Элемент в фокусе (готов принимать ввод)      | -                                   |
...
| `:enabled`           | Элемент доступен для редактирования/кликов   | `//*[not(@disabled)]`               |
| `:disabled`          | Элемент заблокирован (атрибут `disabled`)    | `//*[@disabled]`                    |
| `:read-only`         | Поле только для чтения (`readonly`)          | `//*[@readonly]`                    |
| `:read-write`        | Поле доступно для редактирования             | `//*[not(@readonly or @disabled)]`  |
| `:checked`           | Радио-кнопка или чекбокс отмечены            | `//*[@checked]`                     |
| `:indeterminate`     | Чекбокс в неопределённом состоянии           | -                                   |
| `:required`          | Поле обязательно для заполнения (`required`) | `//*[@required]`                    |
| `:optional`          | Поле необязательное (нет `required`)         | `//*[not(@required)]`               |
| `:valid`             | Значение прошло HTML5-валидацию              | -                                   |
| `:invalid`           | Значение НЕ прошло HTML5-валидацию           | -                                   |
| `:in-range`          | Число в `input[type=number]` внутри min/max  | -                                   |
| `:out-of-range`      | Число вне диапазона min/max                  | -                                   |
| `:placeholder-shown` | Плейсхолдер виден (поле пустое)              | `//*[@placeholder and not(text())]` |
| `:empty`             | Элемент не содержит текста и дочерних узлов  | `//*[not(node())]`                  |
| `:not(selector)`     | Логическое НЕ (исключение)                   | `//*[not(условие)]`                 |
| `:has(selector)`     | Содержит внутри указанный элемент            | `//*[.//вложенный_элемент]`         |
| `:first-child`       | Первый дочерний элемент родителя             | `//*[1]` (в контексте родителя)     |
| `:last-child`        | Последний дочерний элемент родителя          | `//*[last()]`                       |
| `:nth-child(n)`      | Элемент на позиции n (1-based)               | `//*[n]`                            |
| `:nth-of-type(n)`    | n-ный элемент своего типа                    | `//tag[n]`                          |
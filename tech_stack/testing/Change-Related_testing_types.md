...
> Тесты связанные с изменениями - это проверка того, что недавние изменения в коде (исправления, новые функции) работают корректно и не сломали существующий функционал.
...
<details>
  <summary>
    <b>Build Verification Test / BVT (Тестирование сборки)</b>
  </summary>

## Основные понятия

- `Build Verification Test (BVT)` — набор автоматизированных или ручных тестов, выполняемых сразу после создания новой сборки программного обеспечения с целью подтверждения её базовой работоспособности.
- Часто используется как синоним **дымового тестирования**, но с акцентом на **приёмку сборки** в QA-процесс.

## Цели и задачи

- Убедиться, что сборка стабильна и пригодна для дальнейшего тестирования.
- Проверить, что критически важные компоненты запускаются и отвечают без фатальных ошибок.
- Предотвратить трату времени QA-команды на тестирование «битой» или нерабочей сборки.
- Обеспечить быструю обратную связь команде разработки.

## Область применения

- После каждой автоматической сборки в CI/CD pipeline.
- Перед передачей сборки от Dev-команды к QA-команде.
- В nightly- или release-сборках.

## Основной фокус

- Базовая функциональность: запуск приложения, вход в систему, открытие ключевых экранов/модулей.
- Отсутствие критических сбоев: падения, зависания, ошибки развёртывания.
- Минимальная работоспособность всех основных компонентов.

## Скорость выполнения

- Должен быть максимально быстрым — обычно от нескольких секунд до 10–15 минут.
- Как правило, полностью автоматизирован.

## Реакция на ошибки

- При любом провале BVT сборка считается **непригодной** для тестирования.
- QA-команда не приступает к работе с такой сборкой.
- Разработчики получают уведомление и исправляют проблему до следующей сборки.

## Лучшие практики

- Поддерживать минимальный, но репрезентативный набор тестов — только «жизненно важные» сценарии.
- Интегрировать BVT в CI/CD как обязательный quality gate.
- Использовать надёжные, стабильные тесты без внешних зависимостей (или с моками).
- Регулярно актуализировать набор BVT при изменении архитектуры или ключевых пользовательских путей.
- Чётко документировать критерии прохождения/провала.

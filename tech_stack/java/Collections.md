...
> Высокопроизводительная, потокобезопасная хэш-таблица без блокировки всей структуры.
>
> Использует **CAS + synchronized на уровне бакетов** (начиная с Java 8), а не сегментную блокировку (Java 7).
>
> Оптимизирована под высокую конкурентность чтения и умеренную — записи.

- Основа: массив бакетов (`Node<K,V>[] table`), каждый бакет — один из:
  - обычный `Node` (связный список),
  - `TreeBin` (обёртка над красно-чёрным деревом, при ≥8 узлов и размере таблицы ≥64),
  - `ForwardingNode` (указывает на новую таблицу во время resize),
  - `ReservationNode` (для compute-операций).
  
Каждый бакет защищён **локальным synchronized-блоком** (на первом узле бакета) + CAS для создания/замены.

- Порядок: НЕГАРАНТИРОВАН (не является `SequencedMap`)
- Дубликаты ключей: НЕТ; дубликаты значений: ДА
...
- Сложность (амортизированная, при умеренной конкуренции):
  - `get(key)` — O(1), **без блокировок** (volatile-read + final-поля)
  - `put(key, value)`, `remove(key)` — O(1), с **локальной блокировкой одного бакета**
  - `containsKey`, `replace`, `compute` — O(1)
  - `containsValue` — O(n), **может быть неточным** при конкурентной модификации
  - `size()`, `mappingCount()` — O(1) (с Java 8: через `CounterCell[]` + base counter; не требует полной блокировки)
  - `keySet()`, `values()`, `entrySet()` — O(1) (view); итераторы — **weakly consistent**, не fail-fast
- Потокобезопасность: ДА  
  - все операции атомарны по контракту `ConcurrentMap`;  
  - итерация не блокирует, но может не видеть самые свежие изменения;  
  - `ConcurrentModificationException` **никогда не выбрасывается**.

- Особенности внутренней реализации:
  - **resize ленивый и конкурентный**:  
    - при переполнении бакета вызывается `transfer()`, который **делит работу по ресайзу между потоками**;  
    - старые бакеты заменяются на `ForwardingNode`, указывающий на новую таблицу;  
    - новые операции автоматически перенаправляются в новую таблицу.
  - **treeification** происходит **только если таблица уже стабильна** (не в процессе resize);  
    - дерево используется для защиты от DoS при коллизиях (как в `HashMap`).
  - **counter cells**:  
    - `size()` и `mappingCount()` используют распределённый счётчик (`CounterCell[]`) для избежания contention на одном long-поле.
  - **методы compute/merge**:  
    - используют `ReservationNode` для временной блокировки бакета и предотвращения гонок при вычислении значения.
  - **максимальное число сегментов**: теоретически до `table.length` (каждый бакет — независимая критическая секция).
  - **начальная ёмкость**: 16 (как в `HashMap`), но **initialCapacity в конструкторе интерпретируется как ориентир**, а не точное значение.
  - **loadFactor игнорируется** (фиксирован внутренне ~0.75, но управление resize — адаптивное).
  - **не сериализует порядок** — при десериализации порядок элементов может отличаться.

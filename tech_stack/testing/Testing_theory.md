...
```text
ТИПЫ ТЕСТИРОВАНИЯ (Testing Types)
│  Классификация по цели и объекту проверки
│
├─ Functional Testing (Функциональное тестирование)
│  │  Проверка соответствия функциональности системы заявленным требованиям
│  │
│  ├─ Security Testing (Тестирование безопасности)
│  │     Проверка защиты системы от несанкционированного доступа и уязвимостей
│  │
│  ├─ UI/UX Testing (Тестирование пользовательского интерфейса и опыта)
│  │     Оценка удобства, интуитивности и визуальной корректности интерфейса
│  │
│  ├─ Internationalization I18N (Интернационализация)
│  │     Подготовка архитектуры приложения к поддержке множества языков и регионов
│  │
│  ├─ Localization L10N (Локализация)
│  │     Адаптация интерфейса и контента под конкретный язык и культурные особенности
│  │
│  ├─ Globalization G11N (Глобализация)
│  │     Комплексный процесс поддержки I18N и L10N для работы на глобальном рынке
│  │
│  ├─ API Testing (Тестирование API)
│  │     Проверка контрактов, методов, запросов и ответов интерфейсов взаимодействия
│  │
│  ├─ Database Testing / DB Testing (Тестирование баз данных)
│  │     Проверка целостности данных, корректности схем, запросов и транзакций
│  │
│  ├─ Integration Testing (Тестирование взаимодействия / Интеграционное)
│  │     Проверка корректного обмена данными и взаимодействия между модулями или сервисами
│  │
│  ├─ Contract Testing (Контрактное тестирование)
│  │     Валидация соответствия ожиданий потребителя и поставщика сервиса (consumer-driven contracts)
│  │
│  ├─ Regression Testing (Регрессионное тестирование)
│  │     Проверка, что изменения в коде не нарушили ранее работоспособный функционал
│  │
│  └─ End-to-End Testing / E2E (Сквозное тестирование)
│        Проверка полного пользовательского сценария от начала до конца в продакшн-подобном окружении
│
├─ Non-functional Testing (Нефункциональное тестирование)
│  │  Проверка качественных характеристик системы: производительность, надёжность, удобство и т.д.
│  │
│  ├─ Performance Testing (Тестирование производительности)
│  │     Оценка скорости, отзывчивости и стабильности системы под нагрузкой
│  │
│  ├─ Volume Testing (Объёмное тестирование)
│  │     Проверка работы системы с большими объёмами данных в базе или при обработке
│  │
│  ├─ Usability Testing (Тестирование удобства использования)
│  │     Оценка того, насколько продукт понятен, эффективен и удовлетворяет потребности пользователя
│  │
│  ├─ A/B Testing (A/B-тестирование)
│  │     Сравнение двух версий функционала для оценки влияния на поведение пользователей
│  │
│  ├─ Compatibility Testing (Тестирование совместимости)
│  │     Проверка корректной работы системы в различном окружении: ОС, браузеры, устройства
│  │
│  ├─ Cross-Browser / Cross-Platform Testing (Кросс-браузерное / Кроссплатформенное тестирование)
│  │     Частный случай совместимости: проверка веб- или мобильного приложения на разных платформах
│  │
│  ├─ Installation/Deployment Testing (Тестирование установки/развертывания)
│  │     Проверка процессов установки, обновления, настройки и удаления ПО
│  │
│  ├─ Failover and Recovery Testing (Тестирование на отказ и восстановление)
│  │     Проверка способности системы корректно восстанавливаться после аппаратных или программных сбоев
│  │
│  └─ Containerization Testing (Тестирование в контейнерах)
│        Проверка работы приложения в изолированных средах (Docker, Kubernetes), включая конфигурацию и оркестрацию
│
└─ Change-Related Testing (Тестирование, связанное с изменениями)
   │  Проверка корректности модификаций и отсутствия регрессии после внесения изменений в код
   │
   ├─ Smoke Testing (Дымовое тестирование)
   │     Быстрая проверка критического функционала после новой сборки для определения готовности к дальнейшему тестированию
   │
   ├─ Regression Testing (Регрессионное тестирование)
   │     Проверка, что новые изменения не сломали существующий функционал (пересекается с функциональным типом)
   │
   ├─ Sanity Testing (Санитарное тестирование)
   │     Поверхностная проверка конкретной исправленной функции или области перед углублённым тестированием
   │
   └─ Build Verification Test / BVT (Тестирование сборки)
         Набор автотестов, запускаемых сразу после сборки для подтверждения её базовой работоспособности
```

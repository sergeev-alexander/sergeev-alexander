...
### Сигнатура метода `collect()`: `<R, A> R collect(Collector<? super T, A, R> collector)`

где:

`T` - тип элементов в потоке

`A` - тип аккумулятора (внутреннее состояние коллектора)

`R` - тип результата

Альтернативная версия: `<R> R collect(Supplier<R> supplier, BiConsumer<R, ? super T> accumulator, BiConsumer<R, R> combiner)`

...
- Проводите регулярные синхронизации перед стартом спринта, а не после написания кода.
...
- **And / И** и **But / Но** — логические продолжатели предыдущего ключевого слова. Читаются как отдельный шаг.
...
- `asMap()` — преобразование вертикальной (двухколоночной) таблицы в `Map<String, String>`
  - Вертикальная таблица из 2 колонок → `asMap()` возвращает `Map<первая_колонка, вторая_колонка>`
  - Первая колонка становится ключами, вторая — значениями
  - Количество строк не ограничено
  - Если колонок больше двух — выбросит `CucumberException`
- `asMaps()` — преобразование всех строк таблицы в List<Map<String, String>> (каждая строка — отдельный map)
...
// Или использовать Records:
public record OrderItem(
        String productName,     // маппится с "product_name"
        int itemQuantity,       // маппится с "item_quantity"  
        BigDecimal unitPrice    // маппится с "unit_price"
) {}
...
Сторонняя библиотека `cucumber-datatable-to-bean-mapping` предоставляет аннотацию `@Column`, 
которую можно использовать, если имена совсем не совпадают:
...
```java
    ...
    public void createOrder(@Transpose Order order) {
        // Транспонирование вертикальной таблицы: строки → колонки
    }
}
```
...
> Аннотация указывает Cucumber использовать аннотированный метод если в шаге есть таблица и ожидается объект определённого класса.
> 
> Когда Cucumber сканирует glue-пакеты, он находит методы с `@DataTableType` и регистрирует их во внутреннем реестре `TypeRegistry`.
>
> Ключом для поиска служит возвращаемый тип метода (OrderItem в вашем примере).


Используйте ручной маппер, если:

- Имена колонок в таблице не совпадают с полями класса
- Нужно распарсить нестандартный формат (дата, валюта, процент)
- Требуется валидация данных прямо при чтении таблицы
- Объект имеет сложную структуру (вложенные объекты, коллекции внутри ячейки)
- Нужно обработать null или пустые значения особым способом

```gherkin
Scenario: Add items to cart
  Given order contains items:
    | product | qty | price   |
    | Laptop  | 1   | $999.99 |
    | Mouse   | 2   | $29.99  |
```
...
// 1. Регистрация маппера (вызывается Cucumber автоматически при старте)
// Метод с @DataTableType не обязан находиться в классе с шагами (@Given/@When). 
// Его можно вынести в отдельный класс-конфигурацию:
public class DataTableMappers {
  
    @DataTableType
    public OrderItem orderItem(Map<String, String> row) {
        // Можно кастомизировать параметры (удаляем валюту и запятые перед парсингом):
        String priceRaw = row.get("price"); // "$999.99"
        String priceClean = priceRaw.replaceAll("[$.]", "");
      
        // Можно валидировать параметры прямо в маппере:
        int qty = Integer.parseInt(row.get("qty"));
        if (qty <= 0) {
            throw new CucumberException("Quantity must be positive, got: " + qty);
        }
      
        // Если имена колонок не совпадают с именами полей класса, то можно парсить объекты вручную:
        return new OrderItem(
                        row.get("product"),         // product → productName
                        qty,                        // qty → itemQuantity
                        new BigDecimal(priceClean)  // price → unitPrice
        );
    }
}
...
public class OrderSteps {
    ...
    // Теперь вы не работаете с Map вручную
    @Given("order contains items:")
    public void orderContainsItems(List<OrderItem> items) { // DataTable автоматически конвертируется через @DataTableType
        // Здесь уже готовый List<OrderItem>
...
```java
// Cucumber сам находит статические/нестатические методы с @DocStringType при сканировании, если класс находится в glue-пакете
...
# 8. Параметризация: Scenario Outline и Examples

> Параметризация позволяет запускать один и тот же сценарий с разными входными данными, избегая дублирования шагов. 
> 
> `Scenario Outline` в сочетании с `Examples` — основной механизм data-driven тестирования в Cucumber.
...
Feature: Login functionality
  ...
  Scenario Outline: User login with different credentials
    ...
    Examples: Valid credentials
      ...
      | user1    | pass456  | success |

    Examples: Invalid credentials
      | username | password   | status |
      | hacker   | wrong_pass | error  |
      |          | 12345      | error  |
```
...
public class LoginSteps {
    ...
    // Параметр автоматически извлекается из Gherkin-плейсхолдеров
    // Cucumber маппит строку из Examples в аргументы метода по порядку вызова сценария
    @Given("the user navigates to the login page")
    public void navigateToLoginPage() {
        // Инициализация страницы или переход на URL (выполняется для каждой строки Examples)
        LoginPage.open();
    }

    @When("they enter username {string} and password {string}")
    public void enterCredentials(String username, String password) {
        // Автоматическая подстановка значений из таблицы Examples
        // Например: username="admin", password="admin123" для первой строки
        // username="hacker", password="wrong_pass" для третьей строки
        LoginPage.fillLogin(username);
        LoginPage.fillPassword(password);
    }

    @Then("the system returns status {string}")
    public void verifyStatus(String expectedStatus) {
        // Проверка результата: ожидание элемента, верификация текста или URL
        // Значение expectedStatus берётся из колонки "<status>" текущей строки Examples
        Assert.assertEquals(DashboardPage.getStatus(), expectedStatus);
    }
}
```
...
## Комбинация с тегами для селективного запуска

- Теги на Examples наследуются от `Scenario Outline`, но могут быть уточнены для конкретных таблиц
- Фильтрация в CLI — позволяет запускать только конкретные наборы данных: `@boundary`, `@smoke-examples`
- Приоритет — теги на строке `Examples:` применяются ко всем строкам этой конкретной таблицы

```gherkin
@api
Scenario Outline: Process payment via different methods
  Given payment service is available
  When user selects payment method "<method>"
  And submits amount "<amount>"
  Then transaction status is "<status>"

  @smoke @credit-card
  Examples: Credit Card
    | method      | amount | status    |
    | credit_card | 100.00 | approved  |
    | credit_card | 0.50   | approved  |
...
```bash
# Запуск только кредитных карт (smoke)
mvn test -Dcucumber.filter.tags="@credit-card"

# Запуск всех crypto-тестов, но не smoke
mvn test -Dcucumber.filter.tags="@crypto and not @smoke"
```

### Ограничения: Scenario Outline + DataTable

- Плейсхолдеры в `DataTable` — можно использовать `<param>` внутри ячеек таблицы, но это усложняет парсинг и снижает читаемость feature-файлов
- Один `Examples` на шаг — нельзя параметризовать несколько независимых таблиц разными наборами в одном `Scenario Outline`
- Производительность — большие таблицы `Examples` (сотни строк) замедляют запуск и раздувают отчёты
- Рекомендация — для сложной структуры данных используйте `DocString` или `DataTable` в обычном `Scenario`, а данные генерируйте программно

```gherkin
Feature: Order processing

  # Плейсхолдер внутри DataTable — допустимо, но требует осторожности
  # Cucumber сначала подставит значения из Examples, затем передаст результат в DataTable
  Scenario Outline: Place order with dynamic items
    Given the cart is empty
    When user adds items:
      | item_name   | quantity | price  |
      | <product>   | <qty>    | <cost> |
    Then total order amount is "<total>"

    Examples: Single product orders
      | product | qty | cost  | total |
      | Laptop  | 1   | 999.0 | 999.0 |
      | Mouse   | 2   | 25.0  | 50.0  |
```
...
public class OrderSteps {
    ...
    // Cucumber автоматически разворачивает <param> перед передачей в метод
    // DataTable уже содержит подставленные значения из текущей строки Examples
    @When("user adds items:")
    public void addItems(DataTable dataTable) {
        // dataTable.asMaps() вернёт уже подставленные значения
        // Например: [{item_name=Laptop, quantity=1, price=999.0}]
        List<Map<String, String>> rows = dataTable.asMaps(String.class, String.class);
        
        for (Map<String, String> row : rows) {
            // Парсим строки в нужные типы для передачи в сервис
            String product = row.get("item_name");
            int qty = Integer.parseInt(row.get("quantity"));
            double price = Double.parseDouble(row.get("price"));
            
            // Добавляем товар в корзину тестового контекста
            CartService.add(product, qty, price);
        }
    }

    @Then("total order amount is {double}")
    public void verifyTotal(double expectedTotal) {
        // Проверяем итоговую сумму с точностью до копеек
        assertThat(CartService.calculateTotal()).isEqualTo(expectedTotal);
    }
}
```
...
- Позволяет дочернему (или производному) классу наследовать структуру и поведение родительского (или базового) класса.
...
### Поведение членов класса при наследовании в Java</b>
...
- Описание: Если дочерний класс объявляет поле с тем же именем, что и у родительского класса, это поле скрывает (не переопределяет!) поле родителя.
- Решение о выборе поля: Определяется во время компиляции на основе типа ссылочной переменной.
- Вывод: Выбор скрытого поля зависит от типа ссылки, через которую происходит доступ.
...
- Вывод: Выбор переопределенного метода зависит от типа объекта, на котором вызывается метод.
...
- Вывод: Доступ к статическим членам (методам и полям) определяется типом ссылки или именем класса, через которое происходит вызов, а не типом объекта. Они не участвуют в механизме динамического полиморфизма.
